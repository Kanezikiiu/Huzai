...
for k, (ai, a) in enumerate(idx):
    ...
    while e >= s and lines[e].strip() == "":
        e -= 1
    name = a.split("fun ")[-1].split("(")[0].strip()
    blocks.append((name, s, e))

print("=== blocks (dry) ===")
for name, s, e in blocks:
    print(f"{name:20s} lines {s+1}-{e+1}  ({e-s+1})")
...
if len(sys.argv) > 1 and sys.argv[1] == "apply":
    groups = {
        "ScorePageSchedule.kt": ["ScheduleList", "MatchCard", "TeamScore"],
        "ScorePageWidgets.kt": ["PlayerScoreCard", "EmptySchedule"],
    }
    ...
    print("ScorePage.kt ->", len(keep), "lines")